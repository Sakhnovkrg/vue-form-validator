...
`validateField(name)` использует кэш: результат запоминается по значению поля и значениям
полей-зависимостей. Если правило смотрит **наружу формы** — лимит из профиля, список
из справочника — такая зависимость в ключ кэша не попадает, и обычный вызов вернёт
прошлый результат. Для этого случая есть `force`:
